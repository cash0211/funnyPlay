...
// YYMemoryCache(objectForKey:) -> bundlePath -> imageWithContentsOfFile -> imageByDecoded -> YYMemoryCache(setObject:forKey:)
...
+ (NSArray<WBEmoticonGroup *> *)emoticonGroups {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            ...
            // 如 EmoticonWeibo.bundle/com.sina.default/info.plist -> 字典转model
        ...
        for (NSString *path in additionals) {
            WBEmoticonGroup *group = groupDic[path];  // com.sina.default
            if (!group) continue;
            // EmoticonWeibo.bundle/additional/com.sina.default/info.json
            ...
            // JSON->Model => 额外的6张图.group = groupDic[com.sina.default] + group(insert6)
